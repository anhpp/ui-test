...
public class Calendar_Event_Attachment_Schedule_Participants extends CalendarBase{
	...
	public  void test01_02_AddAndRemoveAttachmentToEvent() {
		...
		clickByJavascript(event.ELEMENT_ADD_EVENT_SAVE_BUTTON);
		/*CaseID: 111864
		...
		clickByJavascript(event.ELEMENT_ADD_EVENT_DELETE_ATTACH.replace("${file}", "English.docx"));
	...
	public  void test03_04_05_AddAParticipantAndSendInvitationAndCheckPrivacy() {
		 ...
		 * If the participants agree to participate (by clickByJavascripting Yes in their received invitation emails), their statuses will be yes.
		...
		clickByJavascript(By.xpath(ELEMENT_GMAIL_TITLE.replace("{$title}", "[Invitation] "+ name)),300000);
		clickByJavascript(By.linkText("Yes"));
		...
		event.goToEditEventForm(name);
		clickByJavascript(event.ELEMENT_PARTICIPANTS_TAB);
		...
		clickByJavascript(By.linkText("Not sure"));
		...
		driver.switchTo().window(mailHandle);
		clickByJavascript(By.linkText("No"));
		...
		event.deleteEventTask(name);
		driver.switchTo().window(mailHandle);
		clickByJavascript(ELEMENT_DELETE_MAIL_2);
	}
	...
	public  void test06_07_CheckAndEditStartendTimeOfEventOnSchedule() {
		...
		waitForAndGetElement("//td[27]").getAttribute("class").contains("userSelection");
		waitForAndGetElement("//td[28]").getAttribute("class").contains("userSelection");
		waitForAndGetElement("//td[29]").getAttribute("class").contains("userSelection");

		dragAndDropToObject(event.ELEMENT_SCHEDULE_DRAG.replace("${index}", "30"), event.ELEMENT_SCHEDULE_DRAG.replace("${index}", "33"));
		waitForAndGetElement("//td[30]").getAttribute("class").contains("userSelection");
		waitForAndGetElement("//td[31]").getAttribute("class").contains("userSelection");
		waitForAndGetElement("//td[32]").getAttribute("class").contains("userSelection");
		waitForAndGetElement("//td[33]").getAttribute("class").contains("userSelection");
...
//		assert waitForAndGetElement(event.ELEMENT_SCHEDULE_TO_TIME_INPUT).getAttribute("value").equals("08:00") : "Wrong update to time";
		...
		clickByJavascript(event.ELEMENT_ADD_EVENT_DETAIL_TAB);
